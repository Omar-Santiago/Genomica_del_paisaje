...
vcfR_0 <- read.vcfR("F:/Gih/Genomica_del_paisaje/salida_cluster_nuevo/populations.snps.vcf")
vcfR_0
...
miss<assess_missing_data_pca(vcfR=vcfR_5, popmap = popmap, thresholds = .75, clustering = FALSE)
...
lista_eliminar_1<-all_loci[!all_loci$ID %in% lista_reducida_1$loc.names,]
write.csv(lista_eliminar_1$ID, "loci_eliminados_hw.csv")

# creamos una lista con los individuos eliminados:
...
# vcftools --vcf /mnt/f/Gih/Genomica_del_paisaje/salida_cluster_nuevo/populations.snps.vcf --remove todas_placa_ind_eliminados.txt --recode --recode-INFO-all --out bosque_limpios
...
plotdapc <- scatter(dapc_paisaje$DAPC, mstree = T, clabel = T, lwd = 2, grid=T, cex=4, 
                    cleg = 0.80, posi.da = "bottomleft" )
